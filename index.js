...
app.get('/logado', (req, res) => res.sendFile(path.join(__dirname+'/logado.html')));
...
app.get("/start", async (req, res, next) => {
  ...
  if (["CONNECTED", "QRCODE", "STARTING"].includes(session.state)) {
    ...
    if (session.status === 'isLogged' || session.status === 'inChat'){
      res.redirect("/logado") //Com esse redirecionamento vc consegue passar pela rota de qrcode sem precisar digitar no navegador
    }
...
app.get("/dialog", async function(req, res, next) {
  var result = await Sessions.setup(sessionName = 'session1');
  res.json(result);
});//dialog
